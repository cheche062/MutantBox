...
//		];
		...
		/**进入战场初始化数据转型*/
		public function initDataTransform(obj):* {
			if (!obj) return new Error('obj 无数据');
		...
		public function updataBlood(hp):void {
			updateData({
				"hp": hp,
		...
		/**npc数据更新*/
		public function updataNpc(index, collect, init_collect, npcId, name, hp, initHp):void {
			updateData({
				"isNpcTeam": true,
				"isBoss": String(npcId) == "10002",
				"name": name,
				"index": index,
				"icon": String(npcId),
				"skin": String(npcId),
		 ...
		 * 
		 */
		public function updateNpcAfterBattle(hp_cost, hp_start, hp_max):void {
