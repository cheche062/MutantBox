...
//			closeOnBlank = true;
		...
		private function renderRightPanel(id):void{
			...
			splitContent.forEach(function(item:String, index:int):void{
				if(item){
					...
					var newChild:ContentItem = new ContentItem(arr[1], url);
					var lastChild:ContentItem = dom_content.getChildAt(dom_content.numChildren - 1);
					if(lastChild){
						newChild.y = lastChild.y + lastChild.height + 15;
					}
					dom_content.addChild(newChild);
				}
			})
